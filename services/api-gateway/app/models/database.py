...
Tables
------
* ``users``              — platform users (analysts, managers, admins)
* ``monitored_entities`` — employees whose behaviour is monitored
* ``alerts``             — risk alerts raised by the ML pipeline
...
The ``audit_logs`` table deserves special attention:
  ...
  **Why immutable?**
  ...
  activity — if an attacker (or a rogue analyst) could modify audit
  ...
  The table has additional columns compared to the v1 schema:
    • ``user_role`` — captures the role at the time of action (in case
      the role is later changed, we preserve what they *were* when the
      action occurred).
    • ``status_code`` — HTTP response code, useful for detecting probing
      attacks (e.g.  floods of 403s).
    • ``response_time_ms`` — latency tracking for performance forensics.
    • ``query_params`` — what filters were applied (e.g.  which entity
      was searched).
    • ``user_agent`` — browser/tool fingerprint for correlation.
"""
...
class User(Base):
    ...
    NOT the monitored employee — that's ``MonitoredEntity``.
    """
    __tablename__ = "users"
...
class AuditLog(Base):
    ...
    **Immutable** audit trail — append-only, no UPDATE / DELETE.
    ...
    Every API call is recorded with:
      • Who — authenticated user id + role at time of request
      • What — HTTP method + path + query parameters
      • When — server-side UTC timestamp
      • Where — client IP address
      • Outcome — HTTP status code + response latency

    Compliance references:
      • GDPR Article 30 (Records of Processing Activities)
      • NIST SP 800-53 AU-3 (Content of Audit Records)
      • NIST SP 800-53 AU-9 (Protection of Audit Information)
      • SOX Section 802 (Alteration of Documents)
    ...
    **investigative asset**:
    ...
    incident the audit log answers critical questions:
      • Did the analyst only view the alerts they were assigned?
      • Did anyone export bulk entity data before the breach?
      • Were thresholds lowered suspiciously before an alert was
        suppressed?
      • What was the exact sequence of API calls the suspect made?

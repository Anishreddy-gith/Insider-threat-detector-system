"""
SIEM Integration Connector — Splunk & Microsoft Sentinel
==========================================================
Pushes high-severity insider-threat alerts to enterprise SIEM
platforms in their native ingestion formats.

Why push to SIEM?
  • SOC teams already live in Splunk / Sentinel — meeting them where
    they work reduces the mean time to acknowledge (MTTA).
  • SIEM correlation engines can cross-reference our alerts with
    network IDS, EDR, and DLP signals for richer context.
  • Compliance frameworks (SOC 2, ISO 27001) require centralised
    logging — the SIEM is the system of record.

Supported targets
-----------------
1. **Splunk HTTP Event Collector (HEC)** — pushes JSON events to
   ``/services/collector/event`` with an HEC token.
2. **Microsoft Sentinel Data Collector API** — pushes JSON to the
...
async def push_to_splunk(alert: dict[str, Any]) -> bool:
    ...
    Failures are logged but never raise — SIEM push is best-effort.
    """
...
def _format_sentinel_payload(alert: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [{
        ...
        # XAI metadata — JSON-encoded for KQL parse_json()

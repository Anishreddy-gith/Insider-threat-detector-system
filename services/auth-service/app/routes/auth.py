"""
Auth route handlers – login, register, refresh, password change, current user.

Security controls:
- Account lockout after N failed attempts (OWASP ASVS §2.2.1)
...
async def register(body: RegisterRequest, db: AsyncSession = Depends(get_db)):
    ...
    if existing.scalar_one_or_none():
        # Generic message – don't reveal which field collided
        raise HTTPException(
...
async def login(body: LoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    2. If account is locked → 423 Locked.
    3. Verify password → on failure, increment counter; on threshold, lock.
    4. On success → reset counter, issue access + refresh tokens.
    """
...
async def refresh(body: RefreshRequest, db: AsyncSession = Depends(get_db)):
    ...
    will fail → revealing the compromise.
    """
    ...
    # ── Family mismatch → token replay detected ─────────────
    if user.refresh_token_family != family:
        # Revoke the entire family by clearing it – forces re-login
...
async def change_password(
    ...
    clearing the family — the user must log in again on other devices.
    """

...
Risk Model:
  â€¢ **Exponential decay**: Risk scores decay towards 0 over time if no
    new anomalies are detected.  This prevents stale alerts from
    permanently stigmatising reformed users.
  â€¢ **Additive anomaly impact**: Each new anomaly increases the risk score
    proportionally to its severity Ã— model confidence.
  â€¢ **Alert deduplication**: Cooldown period prevents alert fatigue from
...
from services.risk_scoring.app.config import get_settings
from services.risk_scoring.app.scoring.risk_engine import RiskEngine
...
app = FastAPI(
    title="ITDS â€” Risk Scoring Service",
    version="1.0.0",
    lifespan=lifespan,
)

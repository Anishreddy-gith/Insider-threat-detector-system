...
# â”€â”€ Lazy singleton clients (initialised in lifespan) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async def init_clients() -> None:
    """Create persistent HTTP clients â€” call during lifespan startup."""
...
async def close_clients() -> None:
    """Gracefully close all HTTP clients â€” call during lifespan shutdown."""
...
# â”€â”€ Generic proxy helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Dashboard aggregation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async def aggregate_dashboard(request_id: str | None = None) -> dict[str, Any]:
    ...
    API-Gateway spec â€” the frontend makes one HTTP call instead of
    three, reducing round-trip latency and connection overhead.

    Fan-out targets:
      1. Risk-scoring  â†’ ``GET /api/v1/thresholds/current``
      2. Alert service â†’ ``GET /api/v1/alerts/stats``
      3. Alert service â†’ ``GET /api/v1/alerts/?status=open&limit=10``
      4. Ingestion     â†’ ``GET /healthz`` (to report ingestion health)

    Any individual failure is caught and returned as ``null`` in the
    merged payload â€” partial degradation is preferable to total failure.
    """

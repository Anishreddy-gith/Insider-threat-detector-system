"""
Kafka Producer â€” Async wrapper
================================
...
Design decisions:
  â€¢ JSON serialisation with Pydantic's ``.model_dump_json()`` ensures all
    payloads are schema-validated before hitting the wire.
  â€¢ Idempotent producer (``enable_idempotence=True``) prevents duplicate
    messages on transient broker failures.
  â€¢ ``acks="all"`` waits for ISR acknowledgement â€” we never want to lose
...
async def publish_event(topic: str, key: str, value: dict[str, Any]) -> None:
    ...
    Args:
        ...
        key:   Partition key (usually ``user_id`` so all events for one
               user land on the same partition â†’ ordering guarantee).

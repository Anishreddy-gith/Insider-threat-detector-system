"""
Ensemble Scorer â€” logistic regression meta-learner.
...
Logistic regression is a *linear* meta-learner, which means:
  ...
  2. It is **fast** (one matrix multiply) â€” latency-critical for
     real-time alerting.
...
class EnsembleScorer(BaseDetector):
    ...
    # â”€â”€ training (calibration) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        Parameters
        ----------
        X : (N, K) â€“ each row is [iso_score, ae_score, lstm_score, gnn_score]
        y : (N,)   â€“ binary labels (1 = confirmed insider, 0 = benign)
        """
        if y is None:
            # No labels â€” mark fitted with default weights only
            self._mark_fitted()
    ...
    def predict(self, X: np.ndarray) -> list[dict[str, Any]]:
        ...
        Parameters
        ----------
        X : (N, K) â€“ stacked anomaly scores from each detector
        """
        if not self.is_fitted:
            raise RuntimeError("Ensemble not fitted â€“ call train() first")

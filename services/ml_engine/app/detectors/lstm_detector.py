...
Best for: **temporal / sequential anomalies** â€” behavioural patterns
that are individually normal but form an unusual *sequence*.  Example:
a user who always logs in â†’ emails â†’ browses â†’ logs out, but one day
follows login â†’ mass-download â†’ USB-mount â†’ logout.

Architecture
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    LSTM(input_dim, hidden=128, layers=2, dropout=0.3)
    â†’ Linear(128, input_dim)

The model is trained as a **next-step predictor**: given the first Tâˆ’1
...
class _LSTMPredictor(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Parameters
        ----------
        x : (B, T, D) â€“ batch of hourly feature sequences

        Returns
        -------
        (B, T, D) â€“ predicted next-step features for each time step
        """
...
class LSTMDetector(BaseDetector):
    ...
    seq_len : int
        Hourly window length (default 24 â€” one day).
    hidden_dim : int
        ...
        **kwargs: Any,
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for batch_in, batch_tgt in loader:
                ...
                loss.backward()
                # Gradient clipping â€” critical for LSTM stability
        ...
        # â”€â”€ Compute training error distribution â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        self._model.eval()
        with torch.no_grad():

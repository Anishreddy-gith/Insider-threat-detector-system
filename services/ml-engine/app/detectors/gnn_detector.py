...
Best for: **relational / lateral-movement anomalies** — detecting when
...
``/restricted/salary_data.xlsx`` — the *point* features are plausible
(normal working hours, moderate file size), but the *graph edge* is
unprecedented.

Graph construction
──────────────────
Bipartite graph: **Users ↔ Resources**

    • User nodes:     behavioural embeddings (D-dim feature vectors)
    • Resource nodes: one-hot type + sensitivity level + access frequency
    • Edge weight:    historical access count in the baseline window
...
class _GraphSAGEAnomalyNet(nn.Module):
    """
    2-layer GraphSAGE → anomaly head.
...
class GNNDetector(BaseDetector):
    """
    GraphSAGE-based relational anomaly detector for User↔Resource graphs.
    ...
    def build_graph(
        ...
        """
        Build a bipartite User↔Resource graph.

        Parameters
        ----------
        user_features : (U, D) – one row per user
        resource_features : (R, D) – one row per resource (padded to D)
        access_matrix : (U, R) – access counts (0 = no edge)

        Returns
        -------
        A ``torch_geometric.data.Data`` object with:
            x           – (U+R, D) node features
            edge_index  – (2, E) bipartite edges (both directions)
            edge_weight – (E,) access-frequency weights
            node_type   – 0=user, 1=resource
        """
    ...
    def predict_with_graph(
        ...
        Parameters
        ----------
        user_features : (U, D) – will replace the first U rows of graph.x

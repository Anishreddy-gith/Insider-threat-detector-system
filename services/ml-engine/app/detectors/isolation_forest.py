...
Best for: **point anomalies** — single events that are statistically
...
splits, producing a shorter average path length ⇒ higher anomaly score.

SHAP integration
────────────────
After each prediction we run ``shap.TreeExplainer`` to produce a
per-feature importance dict.  This tells the SOC analyst *which*
behavioural feature (login_hour, bytes_transferred, …) drove the
...
# ── Feature schema (matches ingestion-service behavioural vectors) ──
FEATURE_NAMES = [
    "login_hour",
    "login_count_24h",
...
class IsolationForestDetector(BaseDetector):
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        ``y`` is ignored — Isolation Forest is unsupervised.
        """
        if X.ndim != 2:
    ...
    def predict(self, X: np.ndarray) -> list[dict[str, Any]]:
        ...
        # sklearn score_samples: lower = more anomalous; range ≈ [-1, 0]
        raw_scores = self._model.score_samples(X_scaled)
        # Normalise to [0, 1]: map min_possible → 1, max_possible → 0
